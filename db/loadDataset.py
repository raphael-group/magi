...
def load_sample_file(filename):
    ...
    header = arrs.pop(0)
    samples = set( arr[0] for arr in arrs )
...
def get_parser():
    ...
    parser.add_argument('-gn', '--group_name', default="Other", required=False, type=str,
        help='Add this dataset to a particular group.')
    parser.add_argument('--make-default', default=False, action='store_true', required=False,
        help='Make datasets of this group checked by default')
...
def run( args ):
    ...
    dbQuery = dict(title=args.dataset_name, is_public=args.is_public, user_id=userID)
    groups = db.datasetgroups.find({"name": args.group_name, "user_id": userID})
    if groups.count() > 0:
        dbQuery['group_id'] = groups[0]['_id']
    else:
        dbQuery['group_id'] = db.datasetgroups.insert({"name": args.group_name, "is_default": args.make_default, "is_public": args.is_public, "user_id": userID})
